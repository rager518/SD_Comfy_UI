...
class ComfyApi extends EventTarget {
  ...
  constructor() {
    super()
    this.api_base = `http://${process.env.NEXT_PUBLIC_SERVER_ADDRESS}` ;//location.pathname.split('/').slice(0, -1).join('/')
    console.log('Running on', process.env.NEXT_PUBLIC_SERVER_ADDRESS)
    //this.initialClientId = sessionStorage.getItem('clientId')
    this.initialClientId = process.env.NEXT_PUBLIC_Client_ID
  }
