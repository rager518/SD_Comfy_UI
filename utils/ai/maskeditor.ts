...
class MaskEditorDialog extends ComfyDialog {
  ...
  createLeftSlider(self, name, callback): HTMLDivElement {
    ...
    divElement.style.lineHeight = '45px'
  ...
  setlayout(imgCanvas: HTMLCanvasElement, maskCanvas: HTMLCanvasElement) {
    ...
    bottom_panel.style.lineHeight = '50px'
  ...
  async setImages(imgCanvas) {
    ...
    // image load
    const filepath = settings.getItem().url;
  ...
  async save() {
    ...
    settings.setItem({ name:`${item.subfolder}/${item.filename}`, url: url });

    const uploadImgSrc = document.getElementById('imgUpload') as HTMLImageElement;
    uploadImgSrc.src = url; 
    uploadImgSrc.srcset=url;
